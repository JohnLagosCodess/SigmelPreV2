$(document).ready(function () {
    // Inicializacion del select2 del listado de servicio y profesional bandeja de Notificaciones
    ...
    //Listado de seleccion profecional bandeja Notificaciones
    ...
    //Listado de servicio de badeja Notificaciones
    ...
    $.ajax({
        ...
        success:function(data) {
            ...
            var modulocalificacionNotifi = '';

            for (let i = 0; i < data.length; i++) {

                if (data[i]['Id_Asignacion'] != '') {
                    actualizarBandeja='<input class="checkbox-class" name="actualizaBandejaNotifi" id="actualizar_id_asignacion_'+data[i]['Id_Asignacion']+'" type="checkbox" value="'+data[i]["Id_Asignacion"]+'">';
                ...
                if (data[i]['Id_Asignacion'] != ''){
                    ...
                    modulocalificacionNotifi = '<form id="form_modulo_calificacion_Notifi_'+data[i]["Id_Asignacion"]+'" action="" method="POST">'+
                                '<input type="hidden" name="_token" value="'+token+'">'+
                                '<input class="btn btn-sm text-info" id="modulo_califi_Notifi_'+data[i]["Id_Asignacion"]+'" value="Modulo Notificaciones" type="submit" style="font-weight: bold; padding-left: inherit;">'+ 
                                ...
                                '</form>';
                    data[i]['moduloNotifi'] = modulocalificacionNotifi;
            ...
            $.each(data, function(index, value){
                capturar_informacion_bandejaNotifi(data, index, value)                  
            })
    ...
    $(".Notifibandeja").hover(function(){
     ...
     function capturar_informacion_bandejaNotifi(response, index, value) {        
         $('#Bandeja_Notifi').DataTable({            
             orderCellsTop: true,
             fixedHeader: true,
             scrollY: 350,
             scrollX: true,
             ...
             "columns":[
                 {
                     ...
                     render: function (data, type, row) {
                         return data.actualizarproser+data.moduloNotifi;
                     }
                 },
                 {"data":"Nombre_Cliente"},
