$(document).ready(function(){
    ...
    $("[name='decision_dictamen_jrci']").on("change", function(){
        ...
        if (opt_concepto_jrci == "Desacuerdo") {
            ...
            if (id_tipo_controversia == 12) {
                ...
                var texto_insertar = "<p>Respetados señores, cordial saludo:</p><p>HUGO IGNACIO GÓMEZ DAZA, identificado como aparece al pie de mi firma, actuando en nombre y representación de SEGUROS DE VIDA ALFA S.A. Aseguradora que expidió el <b><u>seguro previsional a la AFP PORVENIR S.A.</u></b>, debidamente facultado para ello, en atención al dictamen de la referencia, estando dentro de los términos de ley, me permito interponer RECURSO DE REPOSICIÓN Y EN SUBSIDIO DE APELACIÓN ante la Junta, por los siguientes motivos:</p><p>Nuestra inconformidad se dirige a la calificación de PÉRDIDA DE CAPACIDAD LABORAL, dictaminada al (la) afiliado(a) {{$nombre_afiliado}}, {{$tipo_identificacion_afiliado}} {{$num_identificacion_afiliado}} donde califican los diagnósticos: {{$cie10_nombre_cie10_jrci}} por los cuales otorgan puntaje de <b>PCL</b> de <strong>{{$pcl_jrci}} %</strong> con fecha de estructuración del {{$f_estructuracion_jrci}}.</p><p>{{$sustentacion_jrci}}</p><p>Por lo anterior, presentamos el recurso de reposición y en subsidio el de apelación, contra {{$tipos_controversia}}, con el fin que se dictamine el valor correspondiente a las patologías del paciente dando aplicación al Decreto 1507/2014 como normatividad vigente.</p><p>Esperamos haber sustentado claramente nuestra inconformidad, por lo que solicitamos se revoque el dictamen y en su lugar se expida el que se adapte a las circunstancias fácticas del paciente. En caso de que no se revoque, solicitamos se de curso a la apelación ante la Junta Nacional de Calificación.</p><p>Por último, se informa que esta Administradora realizó pago de honorarios pertinentes, el cual se efectuó de manera efectiva y por lo tanto se anexará en los próximos días el soporte del mismo.</p><p style='text-align:center;'>ANEXO:</p><p>Certificado de existencia y representación legal expedido por la Superintendencia Financiera.</p><p style='text-align:center;'>NOTIFICACIONES:</p><p>Recibiré notificaciones en la Carrera 10 # 18 - 36 Edificio Córdoba Piso 4, en la ciudad de Bogotá, D.C.</p><p>Cualquier Información adicional con gusto le será suministrada,</p>";
    ...
    var tabla_comunicados_juntas = $('#listado_comunicados_controversia').DataTable({
        "orderCellsTop": true,
        "destroy": true,
        "autoWidth": false,
        "info": false,
    ...
    $("#listado_comunicados_controversia").on("click",'#editar_comunicado',function(){
    ...
    $("#listado_comunicados_controversia").on('click', "#CorrespondenciaNotificacion", async function() {
    ...
    setTimeout(function() {
        radicados_duplicados('listado_comunicados_controversia');
